...
VALUES ((SELECT id
         FROM exercises
         WHERE owner_ref = usr('therapist@trainer-advisor.pro') AND title LIKE 'Разминка для шеи'),
        0,
        'Встаньте ровно, прижмите пятки к полу, потянитесь макушкой к потолку, почувствуйте как вытягивается позвоночник.'),
       ((SELECT id FROM exercises WHERE owner_ref = usr('therapist@trainer-advisor.pro') AND title LIKE 'Р%'),
        1,
       ...
       ((SELECT id FROM exercises WHERE owner_ref = usr('therapist@trainer-advisor.pro') AND title LIKE 'ПИР на в%'),
        0,
       ...
       ((SELECT id FROM exercises WHERE owner_ref = usr('therapist@trainer-advisor.pro') AND title LIKE 'ПИР на р%'),
        0,
        'Убедитесь, что ваша спина прямая, подбородок направлен назад и вверх.'),
       ((SELECT id FROM exercises WHERE owner_ref = usr('therapist@trainer-advisor.pro') AND title LIKE 'ПИР на р%'),
        1,
       ...
       ((SELECT id FROM exercises WHERE owner_ref = usr('therapist@trainer-advisor.pro') AND title LIKE 'У%'),
        0,
        'Исходное положение может быть лежа на полу, сидя с опорой за спиной или стоя с опорой за спиной.'),
       ((SELECT id FROM exercises WHERE owner_ref = usr('therapist@trainer-advisor.pro') AND title LIKE 'У%'),
        1,
        'Направляйте 4 шейный позвонок к опоре (либо полу), затылком отодвигайтесь от плечей, подбородок направляйте по направлению к ушам.'),

       ((SELECT id FROM exercises WHERE owner_ref = usr('therapist@trainer-advisor.pro') AND title LIKE 'Т%'),
        0,
