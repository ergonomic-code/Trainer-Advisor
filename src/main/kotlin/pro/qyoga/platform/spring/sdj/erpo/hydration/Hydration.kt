...
data class PropertyFetchSpec<T : Any?, V>(
    ...
    val fetchSpec: FetchSpec<*> = FetchSpec(emptyList<PropertyFetchSpec<Any, Any>>())
)
...
data class FetchSpec<T : Any?>(
    ...
    constructor(vararg propertyFetchSpec: KProperty1<T, *>) : this(propertyFetchSpec.map { PropertyFetchSpec(it) })

    constructor(propertyFetchSpec: Iterable<KProperty1<T, *>>) : this(propertyFetchSpec.map { PropertyFetchSpec(it) })
...
fun <T : Any> JdbcAggregateOperations.hydrate(
    ...
    val refs: Map<KProperty1<*, AggregateReference<*, *>?>, Map<Any, Any>> =
        fetchSpec.propertyFetchSpecs.filter { detectRefType(it.property) != null }
